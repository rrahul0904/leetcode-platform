...
from runner import RunnerInputError, parse_request, run_request

MAX_REQUEST_BYTES = 2 * 1024 * 1024
MAX_RESPONSE_BYTES = 512 * 1024
CONCURRENCY = max(1, min(int(os.getenv("RIGOR_RUNNER_CONCURRENCY", "2")), 8))
CAPACITY = BoundedSemaphore(CONCURRENCY)
...
class RunnerHandler(BaseHTTPRequestHandler):
    server_version = "RigorPythonRunner/1"
    ...
    def do_GET(self) -> None:
        ...
        self._json(
            HTTPStatus.OK,
            {"status": "ok", "runtime": "python", "concurrency": CONCURRENCY},
        )
    ...
    def do_POST(self) -> None:
        ...
        try:
            ...
            timeout_seconds = int(self.headers.get("X-Rigor-Timeout-Seconds", "10"))
            if not 1 <= timeout_seconds <= 30:
                raise RunnerInputError("Execution timeout is outside the local policy.")
        except (ValueError, RunnerInputError) as exc:
            self._json(
                HTTPStatus.OK,
                _failure(
        ...
        try:
            ...
            with tempfile.NamedTemporaryFile(
                ...
                result = run_request(parsed, timeout_seconds=timeout_seconds)
...
def main() -> int:
    ...
    port = int(os.getenv("RIGOR_RUNNER_PORT", "8081"))
