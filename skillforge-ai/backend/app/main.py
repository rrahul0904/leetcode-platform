...
def validate_demo_python(code: str) -> None:
    """Fail closed for local demo execution.

    This process-level runner demonstrates the API contract only. Production
    candidate execution must use separately isolated workloads.
    """
    try:
        tree = ast.parse(code)
...
def run_python(payload: PythonRunRequest) -> RunResponse:
    ...
    safe_builtins: dict[str, Any] = {
        ...
        "sum": sum, "min": min, "max": max, "sorted": sorted, "set": set,
        "list": list, "dict": dict, "tuple": tuple, "str": str, "int": int,
        "float": float, "bool": bool, "abs": abs, "all": all, "any": any,
