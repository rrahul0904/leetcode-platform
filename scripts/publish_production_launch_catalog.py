...
requires an explicit one-shot enable flag and reason, accepts exactly the canonical
50 launch IDs, revalidates packages and rights, verifies synchronized database
hashes, and records an audit event for every publication.
"""
...
from content_release_seed import ARCH_SPECS
...
# PY-0001 through PY-0004 predate the deterministic release generator specs but
# are first-party source-controlled launch packages.  The launch contract itself
# has always been 20 Python + 10 SQL + 20 architecture/design questions, so the
# production allowlist must be derived from that canonical allocation rather than
# only from the subset of packages regenerated by content_release_seed.py.
PYTHON_LAUNCH_IDS = frozenset(f"PY-{index:04d}" for index in range(1, 21))
SQL_LAUNCH_IDS = frozenset(f"SQL-{index:04d}" for index in range(1, 11))
ARCHITECTURE_LAUNCH_IDS = frozenset(str(spec[0]) for spec in ARCH_SPECS)


def launch_ids() -> frozenset[str]:
    identifiers = PYTHON_LAUNCH_IDS | SQL_LAUNCH_IDS | ARCHITECTURE_LAUNCH_IDS
    if len(PYTHON_LAUNCH_IDS) != 20:
        raise RuntimeError("Production launch must contain exactly 20 Python IDs")
    if len(SQL_LAUNCH_IDS) != 10:
        raise RuntimeError("Production launch must contain exactly 10 SQL IDs")
    if len(ARCHITECTURE_LAUNCH_IDS) != 20:
        raise RuntimeError(
            "Production launch must contain exactly 20 architecture/design IDs; "
            f"found {len(ARCHITECTURE_LAUNCH_IDS)}"
        )
