...
def scalar(connection: Connection, sql: str, params: dict[str, object]) -> int:
...
def main() -> int:
        ...
        with engine.connect() as connection:
            questions = scalar(
                ...
                WHERE EXISTS (
                    ...
                    WHERE v.question_id=q.id AND v.version=:version
                )
                """,
                params,
            )
            versions = scalar(
                connection,
                "SELECT count(*) FROM question_versions WHERE version=:version",
                params,
            )
            solutions = scalar(
                connection,
            ...
            explanations = scalar(
                ...
                JOIN question_versions v ON v.id=s.question_version_id
                WHERE v.version=:version
                  AND length(btrim(COALESCE(s.explanation,'')))>0
                """,
                params,
            )
            unique_ids = scalar(
                connection,
                """
                SELECT count(DISTINCT q.external_id)
                FROM questions q
            ...
            published = scalar(
                ...
                WHERE v.version=:version AND v.state='published'::content_state
                """,
                params,
            )
            runnable = scalar(
                ...
                WHERE v.version=:version
                  AND COALESCE((v.structured_content->>'runnable')::boolean,false)
                """,
                params,
            )
            public_tests = scalar(
            ...
            search_documents = scalar(
                ...
                SELECT count(*)
                FROM question_versions v
                WHERE v.version=:version AND v.search_document IS NOT NULL
                """,
                params,
            )
            subject_rows = connection.execute(
                text(
                    """
                    SELECT COALESCE(v.structured_content->>'subject','unknown'), count(*)
                    FROM question_versions v
                    WHERE v.version=:version
            ...
            seniority_rows = connection.execute(
                text(
                    """
                    SELECT v.expected_seniority, count(*)
                    FROM question_versions v
                    WHERE v.version=:version
                    GROUP BY 1 ORDER BY 1
                    """
            ...
            index_rows = connection.execute(
                text(
                    ...
                    WHERE schemaname=current_schema()
                      AND tablename IN (
                        'questions','question_versions','question_skills','skills'
                      )
                    ORDER BY indexname
                    """
                )
            ...
            filter_checks = {
                "python": scalar(
                    connection,
                    """
                    SELECT count(*) FROM question_versions v
                    WHERE v.version=:version
                      AND v.structured_content->>'subject'
                          IN ('Python','Python Coding')
                    """,
                    params,
                ),
                "sql": scalar(
                ...
                "snowflake": scalar(
                    ...
                    SELECT count(*) FROM question_versions v
                    WHERE v.version=:version
                      AND v.structured_content->>'subject'='Snowflake'
                    """,
                    params,
                ),
                "data_engineering": scalar(
                    connection,
                    """
                    SELECT count(*) FROM question_versions v
                    WHERE v.version=:version
                      AND v.structured_content->>'subject'='Data Engineering'
                    """,
                    params,
                ),
                "ai": scalar(
                    ...
                    SELECT count(*) FROM question_versions v
                    WHERE v.version=:version
                      AND v.structured_content->>'subject'='AI Architecture'
                    """,
                    params,
                ),
            }
            search_hit = scalar(
                ...
                SELECT count(*) FROM question_versions v
                WHERE v.version=:version
                  AND v.search_document @@ websearch_to_tsquery('english','python')
                """,
                params,
            )
    finally:
    ...
    if runnable != public_tests or runnable != hidden_tests:
        failures.append(
            "runnable test coverage mismatch: "
            f"runnable={runnable}, public={public_tests}, hidden={hidden_tests}"
        )
    if search_documents != args.expected:
        failures.append(
            f"search_document: expected {args.expected}, found {search_documents}"
        )
    if search_hit == 0:
