...
class ExecutionControllerSettings:
    ...
    def discover(cls) -> ExecutionControllerSettings:
        ...
        stale_queued_seconds = int(os.getenv("RIGOR_EXECUTION_STALE_QUEUED_SECONDS", "120"))
        if lease_seconds < 10:
        ...
        if stale_queued_seconds < 30:
            raise RuntimeError("RIGOR_EXECUTION_STALE_QUEUED_SECONDS must be at least 30.")
        return cls(
...
class ExecutionController:
    ...
    def _process_requested(
        ...
        if package.language not in {"python", "sql"}:
            return self._persist_infrastructure_failure(
                package,
                error_category="unsupported_execution_language",
                candidate_message="This runtime is not available in the production executor.",
            )

        handle = self._start_sandbox(package)
        if handle is None:
            return True
    ...
    def _start_sandbox(self, package: DispatchPackage) -> SandboxHandle | None:
        ...
        if package.language == "python":
            ...
            profile_name = str(package.limits.get("profile") or "sql-small")
            handle = self.sandbox.create_sql_execution(
                execution_id=package.execution_id,
                request_payload=request_payload,
                profile_name=profile_name,
            )
        else:
            raise TrustedResultError("Unsupported execution language.")
    ...
    def _monitor(
        ...
        while True:
            ...
            if observation.state in {"SUCCEEDED", "FAILED"}:
                ...
                return self._persist_observation(package, observation, handle)
    ...
    def _persist_observation(
        ...
        try:
            ...
            if (
                ...
                and sandbox_result.error_category in INFRASTRUCTURE_RUNNER_ERRORS
            ):
                return self._defer_infrastructure_retry(
                    package,
                    handle,
                    error_category=sandbox_result.error_category or "runner_infrastructure_error",
                )
            with self.engine.begin() as connection:
        ...
        *,
        ...
        if package.attempt_count >= self.settings.max_attempts:
            return self._persist_infrastructure_failure(
                package,
                error_category="execution_attempt_limit",
                candidate_message="Execution failed after repeated infrastructure errors.",
            )
        self.sandbox.cleanup(handle)
        with self.engine.begin() as connection:
        ...
        if expired is not None:
            logger.warning(
                "execution.infrastructure_retry_scheduled",
                extra={
                    **self._package_log(package, "infrastructure_retry_scheduled"),
                    "error_category": error_category,
                },
            )
        return False
        ...
        with self.engine.begin() as connection:
            connection.execute(
                text(
                    """
                    UPDATE execution_requests
                    SET kubernetes_namespace=NULL,
                        kubernetes_job_name=NULL,
                        lease_owner=NULL,
                        lease_expires_at=NULL
                    WHERE id=:execution_id
                      AND state='CANCELLED'::execution_state
                    """
                ),
                {"execution_id": event.execution_id},
            )
    ...
    def reconcile_once(self) -> int:
        repaired = self._requeue_stale_queued()
        with self.engine.begin() as connection:
        ...
        repaired += self._cleanup_terminal_orphans()
        return repaired

    def _requeue_stale_queued(self) -> int:
        with self.engine.begin() as connection:
            rows = connection.execute(
                text(
                    ...
                    SET published_at=NULL,
                        ...
                        last_error='stale_queued_republish'
                    FROM execution_requests er
                    WHERE eo.aggregate_type='execution'
                      AND eo.aggregate_id=er.id
                      AND eo.event_type='execution.requested'
                      AND er.state='QUEUED'::execution_state
                      AND eo.published_at IS NOT NULL
                      AND eo.published_at < CURRENT_TIMESTAMP
                          - (:stale_seconds * INTERVAL '1 second')
                    RETURNING eo.aggregate_id
                    """
                ),
                {"stale_seconds": self.settings.stale_queued_seconds},
            ).all()
        return len(rows)
    ...
    def _reconcile_lease(self, lease: ExpiredExecutionLease) -> bool:
        ...
        if observation.state == "MISSING":
            package = self._retry_expired_missing_sandbox(lease)
            if package is None:
                package = self._reacquire_expired_execution(
                    lease,
                    observation,
                    namespace,
                    job_name,
                ...
                if terminal:
                    self.sandbox.cleanup(handle)
                return terminal
            return self._start_sandbox(package) is not None

        package = self._reacquire_expired_execution(lease, observation, namespace, job_name)
        if package is None:
            return False
        if observation.state in {"SUCCEEDED", "FAILED"}:
            terminal = self._persist_observation(package, observation, handle)
            if terminal:
                self.sandbox.cleanup(handle)
            return terminal
        return True

    def _retry_expired_missing_sandbox(
        self,
        lease: ExpiredExecutionLease,
    ) -> DispatchPackage | None:
        if lease.attempt_count >= self.settings.max_attempts:
            return None
        ...
        with self.engine.begin() as connection:
            attempt = ExecutionClaimRepository(connection).retry_missing_sandbox(
                lease.execution_id,
                expected_attempt=lease.attempt_count,
                worker_id=self.settings.worker_id,
            ...
            if package.attempt_count != attempt:
                raise RuntimeError("Recovered execution attempt does not match durable claim.")
            return package
    ...
    def _cleanup_terminal_orphans(self) -> int:
        with self.engine.begin() as connection:
            rows = (
                connection.execute(
                    text(
                        """
                        SELECT id, kubernetes_namespace, kubernetes_job_name
                        FROM execution_requests
                        WHERE state IN (
                            'COMPLETED'::execution_state,
                            'FAILED'::execution_state,
                            'TIMEOUT'::execution_state,
                            'CANCELLED'::execution_state
                        )
                          AND kubernetes_job_name IS NOT NULL
        ...
        for row in rows:
            ...
            namespace = str(row["kubernetes_namespace"] or self.sandbox.config.namespace)
            job_name = str(row["kubernetes_job_name"])
            self.sandbox.cleanup(
                SandboxHandle(
            ...
            cleaned += int(updated is not None)
        return cleaned
